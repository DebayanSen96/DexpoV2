...
async function main() {
  ...
  if (!state.adapterIds?.mockSwap) {
    const nm = new ethers.NonceManager(deployer);
    const adapter = (await ethers.getContractAt("contracts/v3/testnet/sepolia-testnet/modules/SepoliaTestnetMockSwapAdapter.sol:SepoliaTestnetMockSwapAdapter", mockSwapAdapter)).connect(nm);
    const hub = (await ethers.getContractAt("contracts/v3/testnet/sepolia-testnet/modules/SepoliaTestnetSwapHub.sol:SepoliaTestnetSwapHub", swapHub)).connect(nm);
  ...
  if (!state.adapterIds?.mockLending) {
    const nm = new ethers.NonceManager(deployer);
    const adapter = await ethers.getContractAt("contracts/v3/testnet/sepolia-testnet/modules/SepoliaTestnetMockLendingAdapter.sol:SepoliaTestnetMockLendingAdapter", mockLendingAdapter);
    const hub = await ethers.getContractAt("contracts/v3/testnet/sepolia-testnet/modules/SepoliaTestnetLendingHub.sol:SepoliaTestnetLendingHub", lendingHub);
  ...
  if (!state.completedSteps?.factoryRegistered) {
    await delay(3000);
    const nm = new ethers.NonceManager(deployer);
    const core = (await ethers.getContractAt("contracts/v3/testnet/sepolia-testnet/core/SepoliaTestnetProtocolCore.sol:SepoliaTestnetProtocolCore", protocolCore)).connect(nm);
