import { ethers } from "hardhat";
import * as fs from "fs";
import * as path from "path";

const BASE_MAINNET = {
  USDC: "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913",
  WETH: "0x4200000000000000000000000000000000000006",
};

async function main() {
  const [deployer] = await ethers.getSigners();
  const state = JSON.parse(
    fs.readFileSync(path.join(__dirname, "..", "deployments", "v3-latest", "base-mainnet.json"), "utf8")
  );

  const swapHub = await ethers.getContractAt(
    "contracts/v3/mainnet/modules/swap/SwapHub.sol:SwapHub",
    state.swapHub
  );
  const vault = await ethers.getContractAt(
    "contracts/v3/mainnet/vault/IndexSwapV3.sol:IndexSwapV3",
    state.testVault.indexSwap
  );
  const weth = await ethers.getContractAt("@openzeppelin/contracts/token/ERC20/IERC20.sol:IERC20", BASE_MAINNET.WETH);

  const vaultWeth = await weth.balanceOf(state.testVault.indexSwap);
  console.log("Vault WETH:", ethers.formatUnits(vaultWeth, 18));

  console.log("\nChecking allowance...");
  const allowance = await weth.allowance(state.testVault.indexSwap, state.swapHub);
  console.log("WETH allowance to SwapHub:", ethers.formatUnits(allowance, 18));

  console.log("\nTrying static call to swapWithSlippage...");
  try {
    const result = await swapHub.swapWithSlippage.staticCall(
      state.testVault.indexSwap,
      BASE_MAINNET.WETH,
      BASE_MAINNET.USDC,
      vaultWeth,
      100
    );
    console.log("Static call result:", result.toString());
  } catch (e: any) {
    console.log("Static call error:", e.reason || e.message);
    if (e.data) console.log("Error data:", e.data);
  }

  console.log("\nApproving and trying again...");
  await (await vault.approveToken(BASE_MAINNET.WETH, state.swapHub, vaultWeth)).wait();
  
  const newAllowance = await weth.allowance(state.testVault.indexSwap, state.swapHub);
  console.log("New WETH allowance:", ethers.formatUnits(newAllowance, 18));

  try {
    const result = await swapHub.swapWithSlippage.staticCall(
      state.testVault.indexSwap,
      BASE_MAINNET.WETH,
      BASE_MAINNET.USDC,
      vaultWeth,
      100
    );
    console.log("Static call result after approval:", result.toString());
  } catch (e: any) {
    console.log("Static call error after approval:", e.reason || e.message);
  }
}

main()
  .then(() => process.exit(0))
  .catch((error) => {
    console.error("Error:", error);
    process.exit(1);
  });
